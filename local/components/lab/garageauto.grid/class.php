...
class GarageautoGridComponent extends \CBitrixComponent implements Controllerable
{
    ...
    private function prepareGridData(): void
    {
       ...
       // $arFilterRes = array_merge(['ID' => $autoIds],$arFilter);

        if (!empty($autoIds)) {
            $autos = AutosTable::getList([
                'filter' => ['ID' => $autoIds,'CONTACT_ID'=> $this ->arParams['CONTACT_ID']] + $filter,
                'select' => [
                    'ID',
                    'MARKA',
                    'MODEL',
        ...
        while ($auto = $autos->fetch()) {
            ...
            if (!isset($groupedGarages[$autoId])) {
                $groupedGarages[$autoId] = [
                    ...
                    'MARKA' => $auto['MARKA'],
                    'MODEL' => $auto['MODEL'],
                    'YEAR_CREATED' => $auto['YEAR_CREATED'],
                    'COLOR' => $auto['COLOR'],
                    'MILEGE' => $auto['MILEGE'],
                ];
            }
